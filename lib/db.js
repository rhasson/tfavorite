...
*  - offset: starting index of where to start grabbing elements.  Assumes view using user id
...
DB.prototype.get = function(user_id, params, cb) {
	var args = [],
		ary,
		couch_obj,
		view = params.offset ? 'favorites/by_user_id' : 'favorites/by_date';
		self = this;
	...
	if (params.sort === 'desc') {
		couch_obj = {
			startkey: [user_id, params.offset || {}],
			endkey: [user_id],
			descending: true,
		}
