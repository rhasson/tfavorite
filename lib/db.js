...
function DB () {
	cradle.setup({
        host: process.env.NODE_ENV === 'deployment' ? config.db.host : 'http://127.0.0.1',
        port: process.env.NODE_ENV === 'deployment' ? config.db.port : 5984,
        auth: process.env.NODE_ENV === 'deployment' ? config.db.auth : {},
        cache: false,
        raw: false
	});
...
*/
DB.prototype.get_multi = function(user_id, ids, cb) {
	var self = this, ary,
		multi = rclient.multi();
	...
	if (user_id && typeof user_id === 'string') {
		rclient.mget(ids, function(e, resp) {
			if (!e) {
				ary = '[' + resp.toString() + ']';
				_cb(e, ary, function(err, list) {
...
console..log('saving to couch');
	items.map(function(v) {
		v.user_id = user_id;
		return v;
	});
	db.save(items, function(e, doc) {
		if (e) console..log('Error saving favorites to couch: ', e);
		if (acb) return acb(e);
		else return;
	});
}
