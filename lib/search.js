...
function Search(name, opts) {
	...
	if (this._load) {
		db.zrevrangebyscore([this._indexname, '+inf', '-inf', 'WITHSCORES'], function(e, resp) {
			if (!e && resp) {
				for(var i=0; i < resp.length; i++) {
					self._index.push({id: parseFloat(resp[i+1]).toString(), str: resp[i]});
					i++;
				};
			}
		});
...
Search.prototype.index = function(str, id, cb) {
	...
	db.zadd(self._indexname, id, str, function(e, resp) {
