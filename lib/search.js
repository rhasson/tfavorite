		...
		db.zrevrange([this._indexname, '0', '-1'], function(e, resp) {
			...
			if (!e && resp) {
				...
				for(var i=0; i < resp.length; i++) {
					x = resp[i].split(':');
					self._index.push({id: x[0], str: x[1].split('|')});
				};
			}
		});
	}
...
Search.prototype.query = function(q, cb) {
	...
	function calc (self, q, cb) {
		process.nextTick(function() {
			var newarr = [], arr = [], arr2 = [], coef, def = 0.875, words;

			arr = self._index.filter(function(v) {
				words = v.str ? v.str : [];
				if (words.length) {
					arr2 = words.filter(function(w) {

						if (w.length && q.length) {
							coef = self._distance(w, q[0]);
							return coef >= def;
						}
					});
				}
...
*/
			arr.forEach(function(v) {
				if (newarr.indexOf(v.id) === -1) newarr.push(v.id);
			});
