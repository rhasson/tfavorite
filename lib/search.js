...
var zcard = Q.nfbind(db.zcard)
	, zrevrange = Q.nfbind(db.zrevrange);
...
Search.prototype.query = function(q, cb) {
	var q, ary, self = this, deferred = Q.defer();
	var calc = Q.nfbind(calculate);
	...
	db.zcard(self._indexname, function(e, len) {
		...
		} else deferred.reject(new Error(e));
	});
